    ...
    <List.Section style={[styles.goalList, { flexDirection: 'row', flexWrap: 'wrap' }]}>
      {courseGoals.map((goal, index) => (
        <List.Item
          ...
          style={[styles.goalsContainer, { margin: 5, textAlign: 'center', fontSize: 25 }]}
          right={() => (
            <TouchableOpacity onPress={() => handleDelete(index)}>
              <Image
                style={{ width: 15, height: 15, marginLeft: 20, marginTop: 8 }}
                source={require('./assets/delete-1487-svgrepo-com.svg')} // Change the source to your PNG or JPEG image
              />
            </TouchableOpacity>
          )}
        />
